...
# Em database.py, modifique a classe Client:
...
class Client(Base):
    ...
    master_api_url = Column(String(255), nullable=False)
    master_api_banco = Column(String(100), nullable=False, server_default="MasterSite")
...
class Intent(Base):
    ...
    # Adicionando os campos estruturados que existem no banco de dados
    quick_replies = Column(Text, nullable=True) # Armazena JSON ou texto delimitado
    images = Column(Text, nullable=True) # Armazena URLs de imagem, delimitadas
...
class IntentVariation(Base):
    ...
    preprocessed_variation = Column(Text, nullable=True) # Armazena o texto otimizado para busca
